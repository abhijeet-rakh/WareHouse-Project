...
public class OrderMethodUtility {
	public void generatePieChart(String path,List<Object[]> list) {
		...
		JFreeChart jfc=ChartFactory.createPieChart3D("OrderMode Pie",dpiedataset,true,true, false);
	...
	public void generateBarChart(String path,List<Object[]> list) {
		...
		JFreeChart jfchart=ChartFactory.createBarChart("OrderMethod BAR","Order Mode","Count", bardataset,PlotOrientation.HORIZONTAL,true,true,false);
		       
		//3.Convert JfreeChart to image
		try {
			ChartUtils.saveChartAsJPEG(new File(path+"/resources/PieChart/OrderMethodBarChart.jpg"), jfchart, 250, 250);
		} catch (IOException e) {
			e.printStackTrace();
		}
