...
public void generatePieChart(String path,List<Object[]> list) {
		...
		JFreeChart jfc=ChartFactory.createPieChart3D("PurchaseOrder Pie",dpiedataset,true,true, false);
		
		//Convert JFreeChart into image by using ChartUtilities
		try {
			ChartUtils.saveChartAsJPEG(new File(path+"/resources/PieChart/PurchaseOrderPie.jpg"),jfc, 250, 250);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void generateBarChart(String path,List<Object[]> list) {
		...
		JFreeChart jfchart=ChartFactory.createBarChart("PurchaseOrder BAR","Quality Check","Count", bardataset,PlotOrientation.HORIZONTAL,true,true,false);
		       
		//3.Convert JfreeChart to image
		try {
			ChartUtils.saveChartAsJPEG(new File(path+"/resources/PieChart/PurchaseOrderChart.jpg"), jfchart, 250, 250);
		} catch (IOException e) {
			e.printStackTrace();
		}
